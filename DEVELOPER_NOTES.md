...
- **empire-schemas' envelope module is byte-synced to the gateway's** (2026-07-23, parity-verified incl. the pinned contract hash) after months of dormant divergence. `gateway.core.envelope` remains the ONLY implementation the live publish path uses; `gateway.schemas` no longer re-exports envelope helpers at all (they were dead surface that could hand callers the wrong impl). If the packages ever drift again, the pinned-hash test in `tests/test_envelope.py` catches a gateway-side change; keep empire-schemas synced deliberately, never the reverse.
