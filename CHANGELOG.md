...
- **CI money-path coverage floor restored** (`tests/test_alpaca_trading_router.py`): `gateway/api/alpaca/trading.py` had quietly regressed to 84% statement+branch coverage against the CI-enforced 88% floor after several commits added the shared-account order-ownership fencing (per-symbol fence/claim/mutation lease around create/replace/cancel/close) without matching tests — undetected because the `Lint & Test` job's semgrep step was blocking earlier in the same run. Added tests for every previously-uncovered branch: the ownership-guard factory (Redis-not-configured / singleton reuse), client_order_id validation edge cases, the fencing/freeze exception paths on `create_order`, `replace_order`, `cancel_order`, bulk `cancel_all_orders` (including a double-cancellation race during a shielded ambiguous-mutation freeze), and `close_position`, plus the previously-untested read-only endpoints (`get_positions`, `get_position`, `get_portfolio_history`, `get_clock`, `close_all_positions`). `gateway/api/alpaca/trading.py` now measures 100% statement+branch coverage; `gateway/providers/alpaca/trading.py` is unaffected (99%, unchanged). No production code changed.
