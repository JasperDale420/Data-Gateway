...
- **Alpaca stock-market-data error logging now shares one severity-by-status implementation** (`gateway/providers/alpaca/market.py`). Eight methods (`get_bars`, `get_quotes`, `get_trades`, `get_latest_bars`, `get_latest_trades`, `get_historical_quotes`, `get_snapshots`, `get_auctions`) each re-implemented the same "4xx logs at WARNING, 5xx logs at ERROR" branch inline — one copy was explicitly commented as mirroring `gateway/api/alpaca/common.py`. They now all call the existing shared `log_provider_http_error` helper (`gateway/providers/_errors.py`), already used by the Alpha Vantage and Finnhub providers. Same events and WARN/4xx-ERROR/5xx split; the shared helper's status field is `status_code` rather than the removed inline calls' `status`, so the helper (`gateway/providers/_errors.py`) now emits both `status` and `status_code` on every provider error event it logs, preserving any dashboards/alerts keyed on Alpaca's old field name.

- **The rest of the Alpaca mixins and the NewsAPI provider now use the same shared error-severity helper**, closing the gap the `market.py` unification above left open. `gateway/providers/alpaca/crypto.py` (8 call sites), `options.py` (7), `forex.py` (2), `corporate.py` (1), and `news.py` (1), plus `gateway/providers/news.py` (2), each logged every `httpx.HTTPStatusError` — including client-caused 4xx like a bad symbol or rate limit — unconditionally at `ERROR`, never having picked up the WARN/4xx-ERROR/5xx split CLAUDE.md documents as the repo convention. All 21 call sites now go through `log_provider_http_error` (`gateway/providers/_errors.py`), so a bad-symbol 4xx from crypto, options, forex, corporate actions, or news no longer floods the ERROR log the way the 2026-03 finnhub/alphavantage incident did. Event names and extra fields (e.g. `news_sentiment_error`'s `symbol`) are unchanged; 5xx/timeout/transport failures still log at ERROR exactly as before.
...
- **A close the broker refused because it had already filled no longer locks a trading system out of its own symbol** (`gateway/api/alpaca/trading.py`, `gateway/core/order_ownership.py`). Alpaca's positions endpoint lags its own fills by 30-60 seconds, so a position monitor that re-sends a close during that window gets the order refused — `403 {"code": 40310000, "message": "insufficient qty available"}`, or `422 {"code": 42210000}`. That refusal is an `APIError`, not an `HTTPException`, so on `POST /orders` and `DELETE /positions/{symbol}` it slipped past the ambiguous-outcome branch and hit the catch-all, which assumed a write might still be in flight and froze the symbol's ownership claim. A frozen claim rejects every later write on that symbol with `409 GW-E4301` — including the owner's own risk-reducing close — and only an operator can lift it. On 2026-08-18 that froze twelve option symbols for one trading system in a single session.

  A submission carries broader proof than a cancel: Alpaca either accepts the order and answers with it, or refuses the request before anything reaches the book, so the symbol is exactly as it was reconciled a moment earlier. The named refusals this account produces on a submission — `403 40310000` (insufficient quantity, day-trade buying power, wash trade) and `422 42210000` (validation, including a position-intent mismatch) — now clear the pending-mutation marker, leave the claim unfrozen, and return the broker's own 4xx to the caller. A close whose position the broker no longer holds (`404 40410000`) is treated the same way. The refusals are listed rather than inferred from the status class: Alpaca publishes no universal "rejected before dispatch" guarantee, and a `429` — or a bare `404` — can come from a proxy in front of the broker rather than the broker, so an unlisted or unreadable 4xx keeps the freeze. So does every ambiguous outcome, unchanged: 5xx, timeouts, transport failures, and any reply whose status or body cannot be read. Cancel and replace keep their narrower proof — refusing to cancel says nothing about whether an earlier write is still travelling — so their classification is untouched.

- **A duplicate close of an already-closed position no longer locks the symbol without freezing it** (`gateway/api/alpaca/trading.py`). `DELETE /positions/{symbol}` for a position the broker no longer holds is answered `404 POSITION_NOT_FOUND` — the provider converts the broker's own error before the route sees it. The route correctly declined to freeze that (it is a definitive answer, not an in-flight write), but the pending-mutation marker it had written before the attempt survived, and a claim carrying one rejects every later write on that symbol with the same `409 GW-E4301` a freeze produces, needing the same operator `thaw-claim` to lift. The marker now clears on any sub-500 outcome — the same test the gateway already uses to decide not to freeze — so a symbol it deliberately left unfrozen is no longer locked anyway. Found by adversarial review of the fix above; it predates that fix and affected every duplicate close.

  A refused submission can also leave a claim standing over a symbol the broker holds nothing on, because the claim is taken before the write. The refusal path now drops that claim, but only on proof: the broker is re-read, and the release is skipped unless that read is complete and shows no position and no open order. A resting order in particular keeps its claim — a broker order that outlives its claim is ambiguous for every client and would strand the symbol for all of them. The release is also conditional on the symbol's fence lease still being this request's, checked inside the same atomic script that performs it, so a request whose lease lapsed during those reads cannot delete a claim its successor has since taken. Being unable to prove the symbol is flat is harmless and never freezes: the claim simply stays, and the next submission reconciles it away.

- **A cancel that lost the race to its own fill no longer locks a trading system out of its own position** (`gateway/api/alpaca/trading.py`). When a client cancelled an order that Alpaca had already filled, the broker answered `422 "order is already in \"filled\" state"` — a definitive refusal that changed nothing at the broker. The gateway did not recognise that reply as an order-mutation outcome at all: it fell through to the catch-all handler, which assumed a write might still be in flight and froze the symbol's ownership claim. A frozen claim rejects every subsequent write on that symbol with `409 GW-E4301`, including the owner's own risk-reducing close, and nothing in the request path can lift it. On 2026-08-17 that froze seven option symbols for one trading system, which could not exit any of them for the rest of the session; three were 0DTE and expired worthless. Terminal-state refusals (`42210000` with the `already in "<filled|canceled|expired|rejected|done_for_day>" state` signature) and broker `404`s now clear the pending-mutation marker, leave the claim usable, and return the broker's own 4xx to the caller — which is how a stale cancel learns that its order filled. Genuinely ambiguous outcomes are unchanged and still freeze: 5xx, timeouts, transport failures, unrecognised error bodies, and any other 4xx including the same `42210000` code used for other rejections such as "insufficient qty available". `PATCH /orders/{id}` carried the identical bug and is fixed with it; the batch `DELETE /orders` path already classified these correctly and now has a regression test proving the three paths agree.

  If the pending-mutation marker itself cannot be cleared (Redis fault), the symbol is locked just as a freeze would lock it, so the caller gets the ownership error rather than the broker's 422 — carrying the broker's message so the order state is still visible, plus the `thaw-claim` command that lifts the lock. Reporting the 422 there would tell a caller its order filled and then reject the close it makes in response. That error deliberately does not trigger the usual freeze-on-5xx: it is raised after the symbol's fence is released, and if the store had actually applied the clear and only lost the reply, freezing would stamp a claim another client may already be mutating.

### Added

- **The order-ownership release is now tested against a real Redis and a real expiring fence lease** (`tests/integration/test_order_ownership_lua_integration.py`, run by CI's Redis job, skipped locally when no Redis is reachable at `GATEWAY_TEST_REDIS_URL`). Every other ownership test drives an in-memory stand-in whose scripts are the guard's Lua re-implemented by hand and whose leases never expire, so the one interleaving that most needed proving could not be: a fence lease lapsing between a refused broker write and the release that follows it, while a second request for the same client takes the symbol over. These run the production script itself, let a real lease expire, and include a test that the same release *would* go through if the fence comparison were removed — so the protection cannot be deleted without a test noticing.

- **`python -m gateway.cli thaw-claim <SYMBOL> [--delete]`** — the operator release for a frozen order-ownership claim, which until now had no path out of the code at all and had to be edited by hand in Redis. The command takes the symbol's fence for the whole run — the same lease every Gateway write takes before it writes — so no broker mutation and no second thaw can run beside it; it prints the claim, re-reads the broker, and lifts the freeze and pending-mutation marker only when reconciliation is complete and no open order belongs to anyone but the claim owner. The fence is revalidated immediately before the write, and the write applies only to the exact claim version that was reviewed, so a claim that moved underneath is refused instead of overwritten. `--delete` removes the claim and its owner-index entry outright, and additionally requires the broker to hold no position and no open orders on the symbol. Every thaw logs `order_ownership_thawed_by_operator` at CRITICAL with the symbol, owner, and what was cleared.
...
- **Alpaca data requests use the free-tier feeds** (`config/providers.yaml`, `docker-compose.yml`): the account's market-data subscription no longer includes SIP/OPRA, so every stock/option data call returned 403 "subscription does not permit" — which surfaced downstream as Kairos rejecting 60 of its 63 ENTER decisions with `contract_snapshot_incomplete` on 2026-08-07 (zero orders placed). Stocks now use `feed=iex` (REST and WebSocket), options use `feed=indicative`, and eager stream connect is off because the Basic plan allows only one concurrent WebSocket connection per stream type (verified live: one stocks + one options connection can be held simultaneously). The `/stocks/{symbol}/bars` route also hardcoded `feed=sip` as its query default, overriding the provider's configured feed — it now defaults to the provider-configured feed and `?feed=` remains an explicit override. Quote fidelity note: IEX is a single-exchange view (thinner than SIP NBBO) and indicative option quotes are synthetic; restoring an Algo Trader Plus subscription and reverting these three settings returns full fidelity.
...
- **An open order the broker returns under a malformed symbol no longer drops silently out of ownership reconciliation** (`gateway/api/alpaca/trading.py`). The open-order read is scoped to the symbol being authorized, so every order it returns concerns that symbol — but an order whose symbol would not canonicalise was compared for equality, came back unequal, and was filtered out of the reconciled owner set. The symbol then looked unowned while a live order sat on it, which is exactly the state that lets a second client take the claim. Such an order now fails the reconciliation closed with `unresolvable_broker_order_symbol`.
- **A malformed *position* record no longer reads as "not this symbol" either** (`gateway/api/alpaca/trading.py`, `gateway/core/order_ownership.py`), closing the gap the previous fix deliberately left open (previously tracked in `docs/FOLLOW_UPS.md`). `canonical_broker_symbol()` used to collapse three different outcomes — a genuinely-other asset class, an unparseable symbol, and a malformed human-option strike that raises `decimal.InvalidOperation` (an `ArithmeticError`, not a `ValueError` — previously unhandled and escaping as an uncaught 500) — into the same "skip it" branch. Classification is now positive: `UnsupportedAssetClass` covers only a resolved crypto/forex instrument (a true non-match, since `get_positions()` returns the whole shared account and a single crypto holding must not block every equity order); every resolver or parser failure raises `UnrecognizedBrokerSymbol` instead, which marks the reconciliation incomplete and is rejected by `OrderOwnershipGuard` (`incomplete_broker_reconciliation`). Both subclass `ValueError`, so existing 400/409 handling is unchanged, and no new rejections occur for symbols that resolve normally.
- `test_drain_allows_at_most_32_concurrent_pubacks` no longer depends on machine speed. It waited a fixed 100 event-loop yields for the durable-outbox drain to reach its concurrency ceiling, which is enough on a developer machine and not enough on a loaded CI runner, so the test passed locally and failed in CI. It now waits on an explicit ramp-up signal with a timeout. The assertion is unchanged and still fails both when the ceiling is too low and when it is missing entirely.
- **The durable outbox can no longer abandon an acknowledgement it already received during shutdown** (`gateway/core/durable_outbox_sink.py`). `close()` used to cancel every per-lane drain task unconditionally. If a drain was suspended mid-batch — some rows already acknowledged by the broker, others still publishing — or between the batch finishing and its durable settlement in SQLite, cancelling it there abandoned the already-acknowledged rows: they stayed "pending" and were redelivered on the next start (bounded redundant delivery, deduped downstream by event_id — not data loss, but a real, previously-untested bug in a component every trading system's event delivery depends on). `close()` now waits for each lane to reach a safe point — no acknowledgement outstanding that isn't yet durably settled — before cancelling anything, bounded by a new `drain_shutdown_timeout_seconds` (default 20s, configurable per sink).

  This is a bounded narrowing of the race, not an absolute guarantee: JetStream's client puts no timeout of its own on a publish acknowledgement, so if a broker outage or partition spans the entire shutdown window, the wait still falls back to cancelling — the same behavior this file had before this fix existed at all. That residual only opens during an already-exceptional outage lasting the full configured budget, versus the original bug reproducing on any ordinary shutdown racing an in-flight publish. The alternative (never time out) risks the whole gateway shutdown hanging indefinitely behind a dead broker, which is a worse operational failure mode than bounded, deduped redelivery — this tradeoff was reviewed and the residual explicitly accepted, with a regression test proving it engages only after the configured timeout is genuinely exhausted, not on immediate cancellation.
...
- **Re-recorded the perf baseline for path normalization — this is a real, accepted cost, not runner drift** (`config/perf_baseline.json`): the Perf Gate has failed on master since 2026-08-03 on `test_metrics_path_normalization_baseline` (~0.94s against a 0.705s trend threshold). All perf tests pass and the 1.5s hard budget is not breached; only the trend guardrail trips. The baseline moves 0.45 -> 0.70, putting the threshold at 1.08s — roughly 10% above the worst observed run (0.981s), still well inside the hard budget, and still tripping on any further regression of that size.
  ...
  **The drift is not environmental.** A same-machine A/B over the test's own 200k-path workload shows the symbol matcher introduced on 2026-06-25 (regex, replacing `len<=5 and isupper and isalpha`, so that `BTC-USD`, `BRK.B`, and OCC contracts stop creating a permanent Prometheus series each) normalizes in 0.350s versus 0.252s for its predecessor — a real 1.39x slowdown of exactly the code this test measures, closely matching the 1.43x move in CI from ~0.66s to ~0.94s. That experiment controls for hardware but does not prove CI hardware was constant across April–August, so the precise split between the regex and any runner change is not established; what is established is that treating the whole delta as environmental is wrong. The decision holds either way: the slowdown buys a fix for unbounded metric-label cardinality growth, and the path is memoized, so production pays the regex only on cache misses while this test is a synthetic worst case of 200k *unique* paths (100% misses). The gate was right to catch it — a pass on 2026-06-26 misread it as CI runner variance and loosened only the in-test backstop; that comment is now corrected.
...
- `GET /api/v1/alpaca/positions` now fails closed (502 `GW-E5009`) if Alpaca
  ...
  silently reporting a bogus position count as a success. Matches the
  fail-closed pattern already used for malformed order-list responses.
- The perf trend guardrail can no longer silently relax itself (`scripts/perf_baseline_manager.py`). Baselines feed the allowance `baseline * multiplier + slack`, but the manager overwrote the active baseline with the median of recent *passing* runs in either direction. One slower-but-passing run therefore raised the baseline, which raised the next run's allowance, and so on until the trend guard drifted up to the hard per-test budget and stopped catching anything — for the path-normalization test it would have reached 1.44s against a 1.5s budget after a single passing run. Recent runs may now only ratchet a baseline *down*; raising one is an explicit edit to the checked-in config, which `merge_static_budgets.py` already propagates. This matches the invariant that file's docstring always claimed ("the ratchet system can only tighten budgets and baselines over time") but the manager did not implement.
- **Backfill no longer rejects whole chunks from feeds whose only time evidence is a date** (`gateway/core/backfill.py`). Replay validation reads a record's source timestamp from fields including `filing_date`, `report_date`, `transaction_date`, `effective_date` and `period_end_date` — all date-only. A date-only string parsed to a datetime with no zone attached, which the validator treats as no usable timestamp at all, so the entire provider response for that chunk was refused as `provider_timestamp_missing`. Date-only values now read as UTC midnight, the same reading a real `date` object already got. A date*time* that arrives without a zone is still rejected: its instant is genuinely unknown, where a date's is a convention.
...
- **CI coverage floor on the Alpaca order-placement code path is enforced again.** `gateway/api/alpaca/trading.py` branch coverage was silently at ~84% for months — under the documented 88% floor — because an earlier CI step (semgrep) was broken and blocked everything after it, so the floor never actually ran. With that fixed separately, the gap was real: the Redis-backed ownership-guard lazy-init helper had zero coverage anywhere (a fixture in the main trading-router test file replaces it with a fake for every other test), several read-only routes (`GET /positions`, `GET /positions/{symbol}`, `DELETE /positions`, `GET /portfolio/history`, `GET /clock`) were never called by any test, and the pre-check/post-write ownership-rejection branches on `replace_order`/`cancel_order`/`close_position` only had their `create_order` equivalents covered. New tests (a dedicated file for the ownership-guard init path, plus ~50 additions to the existing router/ownership test files) bring coverage to 100%, comfortably above the floor. Test-only change — no production code modified.
...
- **Shared-account order ownership controls** (`gateway/api/alpaca/trading.py`, `gateway/core/order_ownership.py`, `docker-compose.yml`): Gateway order create, replace, cancel, and position-close requests now reconcile fresh broker positions/open orders before acting, use canonical equity tickers or full OCC contracts, and atomically record non-expiring Redis claims. A durable pre-write marker and synchronous Redis AOF prevent retries after an interrupted broker call. Unknown/manual/mixed/drifted state, incomplete reconciliation, Redis failures, and concurrent Gateway mutations reject without a new broker write. Claims release only after a broker-confirmed flat/no-open-order state.
  ...
  **Scope limits — this is NOT whole-account isolation.** The control only binds order mutations that travel through this Gateway, and only on the five fenced endpoints (`POST /orders`, `PATCH /orders/{id}`, `DELETE /orders/{id}`, `DELETE /orders`, `DELETE /positions/{symbol}`). Known gaps, all fail-closed or explicitly accepted:

  - **Direct broker writers are unaffected.** Kairos and Cerberus also write to Alpaca directly, not through the Gateway. Nothing here constrains them.
  - **`DELETE /positions` (account-wide flatten) is not fenced at all.** It is gated to `super_admin` (`gateway/api/deps.py`), but a super_admin call flattens every client's book without reconciliation, claim, marker, or freeze — including while Redis is unavailable. Treat it as a break-glass operator action, not a routine endpoint.
  - **Claim takeover can race broker visibility lag.** A claim is released and re-takeable by any client once the broker reports the symbol flat with no open orders. If a just-accepted order is not yet visible in `get_orders`, a second client can reconcile the symbol as empty, take the claim, and place its own order on the same symbol. The pending-mutation marker closes this window only while a mutation is in flight, not after it completes.
  - **Frozen claims stay frozen.** Any ambiguous broker outcome freezes the symbol and every later mutation on it rejects until an operator clears the claim by hand. There is no unfreeze workflow yet. A read-only preflight failure (e.g. a `get_positions` timeout) can freeze a symbol another client owns, and a deterministic input-validation 400 raised after the marker is set leaves the marker in place. Both are liveness failures on one symbol; neither lets an unauthorized broker write through.
  - **The Lua scripts are not executed by any test.** The ownership tests drive an in-memory fake that re-implements each script in Python, so the fake and the real Lua must be kept in sync by hand and a divergence between them would not fail the suite. Verifying the scripts themselves (cjson key ordering, decode behaviour on malformed values, Lua truthiness) needs a live Redis.
...
- **CI is green again** (`gateway/core/durable_outbox.py`): the bandit security scan had failed every run since 2026-08-03, blocking the `Lint & Test` check on master and on every branch cut from it. Both findings were `B608` false positives on the outbox's SQL: the `pending()` query interpolates a `WHERE` clause chosen from four hardcoded literals behind a lane allowlist, and the settlement delete interpolates a generated run of `?` bind markers — in both cases every value is bound as a parameter and no caller input reaches the statement. Annotated as reviewed rather than suppressing the rule globally, so a genuine injection elsewhere still fails the build.
...
- **Bulk cancel (`DELETE /orders`) no longer writes to the broker outside the ownership fence** (`gateway/api/alpaca/trading.py`): the route filtered open orders to the caller's own `client_order_id` prefix and then cancelled them straight from `asyncio.gather`, so every one of those cancels reached Alpaca without acquiring the symbol's fence, reconciling the broker, recording a mutation marker, or reconciling afterwards — the one remaining unfenced broker-write path on the shared account. Owned orders are now grouped by canonical broker symbol and each group runs the same fenced protocol as the single-order cancel, holding that symbol's fence for the whole group. Orders on one symbol are cancelled one at a time inside a single acquisition; different symbols still cancel concurrently, so an opening-bell mass cancel does not degrade into a sequential loop. A symbol whose fence cannot be acquired, whose claim is frozen or owned by another client, or whose symbol cannot be canonicalised fails closed — its orders come back as per-order errors carrying the ownership status code and detail, and no unfenced cancel is ever issued as a fallback. One blocked symbol never aborts the others. An ambiguous broker outcome (5xx, an unclassified failure, or the request being cancelled mid-write) freezes that symbol and reports its remaining orders as skipped rather than writing them; if the freeze itself could not be persisted, the response says so instead of claiming the symbol is frozen. A symbol left ambiguous keeps its fence lease until the lease expires instead of handing it straight to the next caller, because the worker thread behind a timed-out cancel can still reach the broker after the request has given up.

  **Known limit carried forward:** the gateway cannot revoke a broker call it has already dispatched. A cancel that times out at the request level may still be executed by its worker thread; the lease now covers that window at the default timeouts, and the frozen claim blocks later gateway mutations permanently, but neither is a live fence around the dispatched call itself. Closing that fully means tying the fence's lifetime to the worker's completion for every fenced route (create, replace, single cancel, and close all share this property today), which is a larger change than this fix. The fence lease is revalidated immediately before every broker cancel — after the upstream Alpaca permit is held, since waiting for that permit is unbounded while the lease is only two minutes — so a group that outruns its lease stops instead of writing to a symbol another client may already hold. An order id the broker reports under two different symbols is never cancelled. Each per-order cancel now runs on the write timeout budget instead of the tighter read one, so a slow-but-successful broker cancel no longer times out into a spurious freeze, and each broker call takes the upstream Alpaca permit for that call alone so a long mass-cancel cannot starve the other trading routes.

- **Settings tests no longer assert against the developer's local `.env`** (`tests/test_config.py`, `tests/test_durable_sink_startup.py`): `gateway/main.py` calls `load_dotenv()` at import time and `tests/conftest.py` imports `gateway.main`, so every `GATEWAY_*` value from a local `.env` is already in `os.environ` before any test runs. Passing `_env_file=None` to `Settings(...)` only disables pydantic-settings' own dotenv read, not the environment, so these tests could pass locally against local config and fail in CI (or silently stop checking the declared defaults). Both files now use an autouse `_isolate_gateway_env` fixture that strips every `GATEWAY_*` key out of the process environment for the module. No assertions needed changing — the defaults they check were already correct.
