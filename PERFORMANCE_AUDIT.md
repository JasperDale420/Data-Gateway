...
1. Continue UW implementation from `PERFORMANCE_AUDIT_UW_DEEP_DIVE.md` (Wave 1 route-helper rollout is now complete across UW routes; next UW focus is provider-side `_call_sync` concurrency gating/metrics and replacing offset over-fetch with native pagination where supported).
