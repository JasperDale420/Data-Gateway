...
- **Finnhub ETF/index cache-telemetry regression tests**: Added `/Users/jacobmcmillan/Empire/Data-Gateway/tests/test_finnhub_etf_router.py` to validate route-level cache hit/miss telemetry for ETF profile and index historical endpoints.

### Changed

- **Finnhub ETF/index route cache telemetry rollout**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/gateway/api/finnhub/etf.py` to emit `record_route_cache(...)` hit/miss metrics across cached ETF/index endpoints.
- **Audit tracker update**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/PERFORMANCE_AUDIT.md` to record completed Finnhub ETF/index telemetry rollout and keep remaining non-provider telemetry follow-up scoped.

## [0.5.120] - 2026-02-08

### Added

- **Finnhub earnings cache-telemetry regression tests**: Added `/Users/jacobmcmillan/Empire/Data-Gateway/tests/test_finnhub_earnings_router.py` to validate route-level cache hit/miss telemetry for earnings calendar and price target endpoints.

### Changed

- **Finnhub earnings route cache telemetry rollout**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/gateway/api/finnhub/earnings.py` to emit `record_route_cache(...)` hit/miss metrics across cached earnings/estimates/recommendations endpoints.
- **Audit tracker update**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/PERFORMANCE_AUDIT.md` to record completed Finnhub earnings telemetry rollout and keep remaining non-provider telemetry follow-up scoped.

## [0.5.119] - 2026-02-08

### Added

- **Finnhub alternative-data cache-telemetry regression tests**: Added `/Users/jacobmcmillan/Empire/Data-Gateway/tests/test_finnhub_alternative_router.py` to validate route-level cache hit/miss telemetry for FDA calendar and congress trading endpoints.

### Changed

- **Finnhub alternative-data route cache telemetry rollout**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/gateway/api/finnhub/alternative.py` to emit `record_route_cache(...)` hit/miss metrics across cached alternative-data endpoints (`fda_calendar`, `congress_trading`, `lobbying`, `usa_spending`).
- **Audit tracker update**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/PERFORMANCE_AUDIT.md` to record completed Finnhub alternative-data telemetry rollout and keep remaining non-provider telemetry follow-up scoped.
...
- **Finnhub analysis cache-telemetry regression tests**: Added `/Users/jacobmcmillan/Empire/Data-Gateway/tests/test_finnhub_analysis_router.py` to validate route-level cache hit/miss telemetry for insider sentiment and pattern recognition endpoints.

### Changed

- **Finnhub analysis route cache telemetry rollout**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/gateway/api/finnhub/analysis.py` to emit `record_route_cache(...)` hit/miss metrics across cached analysis endpoints (`insider_sentiment`, `upgrade_downgrade`, `social_sentiment`, `support_resistance`, `pattern_recognition`).
- **Audit tracker update**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/PERFORMANCE_AUDIT.md` to record completed Finnhub analysis telemetry rollout and keep remaining non-provider telemetry follow-up scoped.
...
- **Forex-rates cache-key regression coverage**: Expanded `/Users/jacobmcmillan/Empire/Data-Gateway/tests/test_alpaca_forex_router.py` to validate cached-helper key/route wiring for live forex-rate reads.

### Changed

- **Live forex short-TTL cache rollout**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/gateway/api/alpaca/forex.py` to use `execute_alpaca_cached_call(...)` for `GET /forex/rates` with normalized pair-key caching and a short TTL.
- **Audit tracker update**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/PERFORMANCE_AUDIT.md` to include live forex-rate cache coverage in selective safe-GET rollout status.
...
- **Forex historical cache-key regression coverage**: Expanded `/Users/jacobmcmillan/Empire/Data-Gateway/tests/test_alpaca_forex_router.py` to validate cached-helper key/route wiring for historical forex queries.

### Changed

- **Forex historical short-TTL cache rollout**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/gateway/api/alpaca/forex.py` to use `execute_alpaca_cached_call(...)` for `GET /forex/rates/historical` with normalized cache keys and bounded TTL.
- **Audit tracker update**: Updated `/Users/jacobmcmillan/Empire/Data-Gateway/PERFORMANCE_AUDIT.md` to include historical forex cache coverage in selective safe-GET rollout status.
