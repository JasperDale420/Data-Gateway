...
The compose Redis (`redis:7-alpine`) runs with AOF persistence (`--appendonly yes --appendfsync always`, since 2026-07-19 after a power outage restarted it empty) and `--maxmemory 6gb --maxmemory-policy volatile-lru` (since 2026-07-20): TTL'd cache/dedup keys are evictable, while the persistent heber streams and consumer groups carry no TTL and are non-evictable — at-cap writes error into the sink failover buffer instead of silently evicting stream data. The instance is unauthenticated and bound to `127.0.0.1` only.
