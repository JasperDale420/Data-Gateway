...
| API routers `gateway/api/alpaca/*` | 14 files (60 endpoints) | COMPLETE | Option-chain snapshot + stock-trades over-fetch reductions, stock snapshot concurrency, shared list-parser rollout, shared execution-helper rollout for stock/options/metadata/forex, metadata safe-GET cache + in-flight dedupe rollout |
...
3. **Alpaca routers**: Continue shared execution-helper consolidation to remaining route modules (`account.py`, `corporate.py`, `crypto.py`, `news.py`, `screener.py`, `trading.py`, `watchlists.py`) and evaluate additional safe-GET cache/dedupe opportunities beyond metadata endpoints.
