...
6. Implement Alpaca Wave 1 optimizations from `PERFORMANCE_AUDIT_ALPACA_DEEP_DIVE.md` (option-chain snapshot and stock-trades over-fetch reductions are complete via provider limit threading; remaining focus is route helper consolidation, cache/dedupe for safe GETs, and snapshot concurrency improvements).
