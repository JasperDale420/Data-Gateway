...
6. Implement Alpaca Wave 1 optimizations from `PERFORMANCE_AUDIT_ALPACA_DEEP_DIVE.md` (option-chain snapshot over-fetch reduction is complete via provider limit threading; remaining focus is route helper consolidation, cache/dedupe for safe GETs, and additional over-fetch reductions).
