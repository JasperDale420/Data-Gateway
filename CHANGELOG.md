...
- The `Lint & Test` CI job is unblocked. Its semgrep step had 9 blocking findings (`empire-no-bare-exception`, `empire-no-return-none-for-failure`) that had accumulated on master as newer code landed. Every site already satisfied the rules' own stated escape hatch — the broad catches log with `exc_info=True` or clean up and re-raise, and the two `None` returns are documented Optional parse/extract helpers whose callers fail closed — but the rule patterns cannot express that, so each now carries a `nosemgrep` annotation with a one-line justification, matching the convention already used elsewhere in the codebase. No behaviour changed: the diff is 9 added comment lines and no deletions.
