...
- CI's "Lint & Test" job is unblocked: the nine blocking `semgrep` findings that had held it red since 2026-08-03 are resolved. Two were real defects and are listed under Fixed; the remaining seven are broad catches at genuine boundaries — cleanup-then-re-raise, a health endpoint that must degrade rather than 500, job/chunk boundaries that fail closed, an optional-dependency probe, and an input validator — each now carrying an individual `# nosemgrep` justification rather than a rule suppression.
...
- **A durable-outbox shutdown failure could skip drain-task cancellation entirely** (`gateway/core/durable_outbox_sink.py`, `_stop_drain`). Adversarial review of the close/drain-race fix above found that the diagnostic pending-count read ran *before* `task.cancel()`; if that read raised (SQLite unavailable, etc.), `close()` propagated the exception without ever cancelling the stuck task, leaking it and aborting shutdown before the outbox connection closed. Cancellation now runs first, unconditionally; the diagnostic read is best-effort afterward and a failure there only drops the row-count telemetry, never the cancellation. (An initial attempt to also bound the diagnostic read with a timeout was reverted after review found it traded this bug for a worse one — `asyncio.to_thread()` offers no real cancellation, so a timed-out read leaves an orphaned worker thread that could still be touching the SQLite connection when `close()` runs; `SQLiteOutbox`'s own internal lock already serializes that safely, so the unbounded-but-ordered-correctly version is the right tradeoff.)
- **`OrderOwnershipGuard` never had a direct test proving it rejects an incomplete broker reconciliation** (`tests/test_order_ownership.py`). The router-layer tests only asserted the classifier's `BrokerSymbolState.complete` value; nothing exercised the guard itself refusing `authorize_submission`/`authorize_close` when `complete=False` — the actual enforcement point for the `UnrecognizedBrokerSymbol` fix above. Added.
- The trading money-path coverage floor (`gateway/api/alpaca/trading.py`, CI-gated at 88%) is met again: closing the outbox close/drain-race fix's tests above pulled the file's own coverage down to 85% with the new `UnrecognizedBrokerSymbol` branches. Nine new unit tests cover the previously-untested classifier and audit-logging branches — object-attribute broker records, a recognized-but-unfenceable asset class reaching `_canonical_order_symbol_or_reject` directly, a malformed non-list broker response, the 500-order truncation cap, a cancel-all order missing its id, and the client-id audit helpers' fallback paths — no behavior changed.
- A durable-outbox perf test (`tests/perf/test_perf_durable_outbox.py::test_outbox_admission_rate_for_one_kib_envelopes`) no longer pays the production 10s `close()` graceful-drain window: its publisher never returns, so nothing is ever mid-settlement and the grace period bought no correctness, only ~10s of avoidable perf-suite wall-clock (enough on its own to blow the Perf Guardrail's suite-runtime threshold). `drain_stop_grace_seconds` is trimmed for this test only; the production default and the rate assertion (computed before `close()` runs) are unchanged.
- **Shutting down the durable outbox no longer abandons a batch the publisher already acknowledged** (`gateway/core/durable_outbox_sink.py`). `close()` cancelled every drain task immediately, with no regard for whether it was mid-flight. If a publish call had already returned success but the SQLite settlement write hadn't landed yet, cancellation skipped that write — the row stayed marked pending and was redelivered as a duplicate on the next start. This is what surfaced two of the pre-existing `test_durable_outbox_sink.py` failures once the CI semgrep gate above stopped blocking the test step from ever running. `close()` now lets an in-flight batch finish (up to a 10s grace period) before cancelling; a batch that's still idle exits on its own once it observes the shutdown. A publisher that never returns still can't hang shutdown forever — past the grace period it's force-cancelled and the row is left pending for redelivery, same as today, now logged with how many rows are at risk.
...
- Added test coverage for `replace_order`'s new unrecognized-original-symbol rejection path (`gateway/api/alpaca/trading.py`), closing the last two untested lines from the shared-account ownership fix below.
- **A broker record the Gateway cannot parse no longer reads as "not this symbol"** (`gateway/api/alpaca/trading.py`, `gateway/core/order_ownership.py`). Shared-account reconciliation scans the whole account's positions and open orders and keeps the ones matching the symbol being fenced. Records it could not resolve — a symbol matching no known format, one missing entirely, or an OCC-shaped contract with an impossible expiry — were being discarded as if they belonged to some other instrument. A live position held by another client could therefore drop out of the scan, present the symbol as flat, and let a second client claim and flatten it. Only a positively identified crypto or forex holding is skipped now; anything unresolvable marks the reconciliation incomplete, which the ownership guard already rejects. No new rejections for symbols that resolve normally.
- A failure while cleaning up after a failed data-sink startup no longer hides the reason startup failed (`gateway/main.py`). If verifying the JetStream outbox failed and closing the outbox then also failed, the close error replaced the verification error in the logs and the publisher was left open. Both close steps now run and log independently, and the original verification error is the one raised.
