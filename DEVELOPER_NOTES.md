...
## Frozen Wire Contracts & the empire-schemas Divergence

- The live envelope path is **only** `gateway/core/envelope.py` — its `EventEnvelope` keys, `compute_event_id` (BLAKE2b-16 over `"|"`-joined `provider|feed|instrument_key|ts_event|<unique fields>`), `FEED_UNIQUE_FIELDS`, and `make_instrument_key` are the wire contract with Heber. Changing join order, separator, digest size, or any feed's unique-field tuple silently changes every `event_id` and breaks Heber's dedup.
- **empire-schemas ships a divergent, dormant duplicate** of `compute_event_id`/`FEED_UNIQUE_FIELDS`/`make_instrument_key` (re-exported as `gateway.schemas.compute_event_id`). Known divergences: `Decimal` stringification (`"1.50"` here vs `"1.5"` there), missing feed disambiguators the gateway added after the insider/congress/oi_change Bronze-collapse incidents, missing newer feeds entirely, and naive crypto pair splitting (`DOGEUSD` → `DOG-EUSD`). **Never consolidate the gateway onto the empire-schemas version** — a regression test pins this (`tests/test_envelope.py`, "divergent duplicate" comment). Nothing on the live publish path imports the empire-schemas copy; keep it that way until the packages are reconciled (see `docs/FOLLOW_UPS.md`).
- The `heber:events` topic literal appears in 9 places (main.py + pollers + backfill + middleware) on purpose — consolidation stays deferred as value-neutral churn: the string is frozen forever, so folding it into a constant buys nothing (see `docs/FOLLOW_UPS.md`).
