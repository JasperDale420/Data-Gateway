...
7. Continue non-provider router follow-ups from `PERFORMANCE_AUDIT_NON_PROVIDER_ROUTERS_DEEP_DIVE.md` (fetcher binding guards, cache-hit-first parsing in news, bulk streaming downloads, replay control-loop remediation, and metrics refresh throttling are complete; remaining focus is list-scaling follow-ups).
