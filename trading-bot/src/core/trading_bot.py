...
from .gateway_client import DataGatewayClient
from .trading_client import TradingClientWrapper
from ..strategies.mean_reversion import MeanReversionStrategy, TradeSignal, Signal
...
class TradingBot:
    ...
    def _init_performance_log(self):
        ...
        if not self.performance_log.exists():
            pd.DataFrame(columns=[
                'timestamp', 'equity', 'cash', 'buying_power', 'positions_count',
                'total_position_value', 'daily_pnl', 'total_pnl'
    ...
    def get_market_data(self, lookback_days: int = 30) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            bars_data = self.data_client.get_stock_bars(
                symbol=self.symbol,
                timeframe='1Day',
                limit=limit
            )
            ...
            if not all(col in df.columns for col in required_cols):
                self.logger.error(f"Missing required columns in bars data")
                return None
    ...
    def analyze_market(self, df: pd.DataFrame) -> Optional[TradeSignal]:
        """
        ...
        try:
            ...
            if signal:
                self.logger.info(f"Signal generated: {signal.signal.value} "
    ...
    def execute_trade(self, signal: TradeSignal) -> bool:
        ...
        try:
            ...
            current_position = next(
                (p for p in self.positions if p['symbol'] == signal.symbol),
                None
            )
            
            if current_position:
                self.logger.info(f"Already have position in {signal.symbol}: "
                               f"{current_position['qty']} shares")
            ...
            self.logger.info(f"Submitting {order_side.upper()} order for {shares} shares "
                           f"of {signal.symbol} at ${signal.price:.2f}")
            self.logger.info(f"Risk: ${risk_metrics['actual_risk']:,.2f} "
                           f"({risk_metrics['actual_risk_percent']:.2f}%)")
