...
1. Continue UW implementation from `PERFORMANCE_AUDIT_UW_DEEP_DIVE.md` (Wave 1 route-helper rollout is complete and provider-side `_call_sync` concurrency gating/metrics is now implemented; next UW focus is replacing offset over-fetch with native pagination support and telemetry-driven inflight tuning).
