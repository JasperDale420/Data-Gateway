...
- **Opening-bell `data_sink_backpressure_drop` storm (silent Heber data loss)** (`gateway/config.py`): The `DataSinkRegistry` in-flight semaphore (`data_sink_max_inflight_per_sink`, default 512) is drop-on-saturation — when 512 publish tasks are already pending, new events are not queued or buffered, they are silently dropped at the semaphore (`gateway/core/data_sink.py:188`). On 2026-05-15 this fired 19 790 times in one minute at 08:59 ET (pre-open) and 11 400 times at 09:36 ET (post-open); 32 757 events lost on the day, of which Heber's Bronze→Silver pipeline received zero. 2026-05-18 saw the same shape (5 872 drops at 13:xx UTC). Distinct failure mode from the 2026-05-05 buffer-eviction outage (`6a54115`) — that one was Redis-side memory pressure; this one is the upstream in-process gate. Raised the default from 512 → 2048 (4× burst headroom); validation cap is 4096 if further tuning is required. Comment in `config.py` documents the failure mode for future operators. Takes effect on next container restart.
