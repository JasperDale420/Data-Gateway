...
11. Continue core modules follow-ups from `PERFORMANCE_AUDIT_CORE_MODULES_DEEP_DIVE.md` (Wave CORE-1 validator hot-path optimization, symbology allocation trimming, quality timestamp/sort reductions, and middleware import hoist are complete; Wave CORE-2 calendar span guardrails plus `live_provider_smoke.py` concurrency and `generate_provider_contract.py` handler pre-indexing are complete; remaining focus is quality single-pass consolidation and security symbol-batch follow-ups).
