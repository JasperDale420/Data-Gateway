...
### P1-4: Over-fetch then trim patterns add avoidable CPU/network work
...
- Option chain snapshot route fetches full chain then truncates to 100:
  - truncation: `gateway/api/alpaca/options.py:96`
  - provider request limit currently `1000`: `gateway/providers/alpaca.py:474`
...
2. Add optional `limit` parameter to `get_option_chain(...)` used by snapshot route.
...
1. Remove over-fetch patterns by propagating route limits into provider methods.
...
| `gateway/api/alpaca/options.py` | 7 | COMPLETE | Chain snapshot over-fetch reduction, helper consolidation |
