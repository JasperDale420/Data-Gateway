...
- **Async-correctness review followups** (`gateway/core/circuit_breaker.py`, `gateway/core/connections.py`, `gateway/core/dedup.py`, `gateway/core/stream.py`, tests): released cancelled half-open circuit probes, closed eager stream start races, deauthenticated slow WebSocket clients before fanout selection, and shielded shared dedup futures from follower cancellation.
- **Events queued before the circuit opens are no longer silently dropped** (`gateway/core/data_sink.py`, `tests/test_data_sink.py`): in-flight `CircuitOpenError` now buffers the event through the registry buffer path.
- **Slow WebSocket clients no longer stall broadcast fanout or upstream reads** (`gateway/core/connections.py`, `tests/test_connections_helpers.py`): outbound sends now have a bounded timeout and slow consumers are closed.
- **Lazy stream connection startup no longer spawns duplicate upstream Alpaca WebSockets** (`gateway/core/stream.py`, `tests/test_multiplexer.py`): dormant restarts now use a per-connection start lock and mark `_running` synchronously.
- **Half-open circuit breakers now allow only one probe at a time** (`gateway/core/circuit_breaker.py`, `tests/test_circuit_breaker.py`): concurrent probes raise `CircuitOpenError`, and probe completion releases the slot.
- **Cancelled dedup leaders no longer hang followers** (`gateway/core/dedup.py`, `tests/test_request_dedup.py`): leader cancellation now cancels the shared future and propagates cleanly.
