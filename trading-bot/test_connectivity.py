...
def test_alpaca_credentials():
    ...
    if api_key and api_secret:
        ...
        try:
            ...
            print(f"✓ Alpaca paper trading account connected:")
            ...
            print(f"  Trying older API...")
            try:
                import alpaca_trade_api as tradeapi
                api = tradeapi.REST(api_key, api_secret, base_url, api_version='v2')
                account = api.get_account()
                print(f"✓ Alpaca paper trading account connected (legacy API):")
                print(f"  Account ID: {account.id}")
                print(f"  Status: {account.status}")
                print(f"  Buying Power: ${float(account.buying_power):,.2f}")
                print(f"  Equity: ${float(account.equity):,.2f}")
...
def test_data_gateway_endpoints():
    ...
    # Use test client credentials from config/clients.yaml
    test_client_key = "gw_test_dev_key_67890"
